...
$$y(t) = \phi^T(t)\theta = \theta\phi^T(t)$$
where
\begin{align*}
\begin{split}
\phi(t) &= \left[\begin{array}{c} u(t) \\ u(t-1) \\ \vdots \\ u(t-n_b) \\ -y(t-1) \\ -y(t-2) \\ \vdots \\ -y(t-n_a) \end{array}\right],
\end{split}
\begin{split}
...
$$\mathcal{S}: y(t) = \phi^T(t)\theta_0$$
where $\theta_0$ is the ''real`` parameter. The model is
$$\mathcal{M}: y(t) = \phi^T(t)\theta + e(t,\theta)$$
where $\theta$ is not necessarily $\theta_0$ but $\phi(t)$ is the same between the system and the model. Notice that the model has an additional error term for when $\theta\neq\theta_0$. Note that $\phi(t)$ is the set of all past inputs and outputs and $\theta$ contains the unknown parameters.
...
\begin{align*}
e(t,\theta) &= y(t) - \phi^T(t)\theta \\
e(1,\theta) &= y(1) - \phi^T(1)\theta \\
e(2,\theta) &= y(2) - \phi^T(2)\theta \\
e(3,\theta) &= y(3) - \phi^T(3)\theta \\
&\vdots
\end{align*}
...
\begin{align*}
\frac{1}{N}\phi^T(\mathbf{e} &= \mathbf{\phi}\mathbf{\theta} - \mathbf{y}) \\
\Rightarrow \frac{1}{N}\phi^Te &= \frac{1}{N}\phi^T\phi\theta - \frac{1}{N}\phi^Ty = 0 \\
\Rightarrow \hat{\theta}_{LS} &= (\frac{1}{N}\phi^T\phi)^{-1}\frac{1}{N}\phi^Ty
\end{align*}
...
It is very interesting, and useful, to note that the term $\frac{1}{N}\phi^Ty$ is the same as the cross-covariance function of the system and the term $\frac{1}{N}\phi^T\phi$ is the same as the auto-covariance function of the system.
