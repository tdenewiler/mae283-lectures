...
\begin{itemize}
\item Data compression -- $n$ points in the frequency domain represent $N>>n$ points in the time domain.% chktex 8
...
\item $\lim_{N\to\infty}E\{[\hat{\Phi}_s^N(\w)-\Phi_s(\w)]^2\} = \Phi_s^2(\w)$.% chktex 3
...
$1-\frac{\tau}{\gamma} \Leftrightarrow \frac{1}{\gamma}\left(\frac{\sin\gamma\w}{\sin\w}\right)^2$, with the time domain on the left and the frequency domain on the right.% chktex 3
